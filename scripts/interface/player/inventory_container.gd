...
@onready var slot_container: GridContainer = $v_container/background/GridContainer
@onready var animation: AnimationPlayer = $animation

@onready var aux_h_container: HBoxContainer = $container/h_container
@onready var aux_animation: AnimationPlayer = $container/animation
...
func _ready() -> void:
	for icon in aux_h_container.get_children():
		icon.connect("mouse_exited", Callable(self, "mouse_interaction").bind("exited", icon))
		icon.connect("mouse_entered", Callable(self, "mouse_interaction").bind("entered",icon))
